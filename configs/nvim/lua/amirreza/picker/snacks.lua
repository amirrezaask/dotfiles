...
vim.api.nvim_create_autocmd("LspAttach", {
  callback = function(args)
    ...
    vim.keymap.set("n", "<leader>so", Snacks.picker.lsp_symbols, { buffer = args.buf, desc = "[s]ymbols (outline)" })
    vim.keymap.set("n", "<leader>sO", Snacks.picker.lsp_workspace_symbols, { buffer = args.buf, desc = "[s]ymbols [w]orkspace" })
  end,
})
