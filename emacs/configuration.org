...
*** Swiper
#+BEGIN_SRC emacs-lisp
(use-package swiper :straight t :commands (swiper) :bind (("C-s" . swiper)))
#+END_SRC
...
** Elixir Setup
#+BEGIN_SRC emacs-lisp
(use-package elixir-mode :straight t :defer t :mode "\\.exs?\\'" 
     :config 
     (message "Need to launch elixir language server manualy what a shame "))
(use-package alchemist :straight t :defer t :mode "\\.exs?\\'")
#+END_SRC
** Rust Setup
#+BEGIN_SRC emacs-lisp
