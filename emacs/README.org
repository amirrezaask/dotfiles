...
- [[#integrated-tools][Integrated Tools]]
  ...
  - [[#application-laucnher][Application Laucnher]]
- [[#dotfiles][Dotfiles]]
...
#+end_src
** Application Laucnher
Dmenu replacement in Emacs.
#+begin_src emacs-lisp
  (defun amirreza/list-of-executables ()
    "List of all executables in `exec-path`."
    (mapcar (lambda (path)
              (ignore-errors (directory-files path t directory-files-no-dot-files-regexp nil))) exec-path))

  (defun amirreza/run-command ()
    "Runs given executable."
    (interactive)
    (let* ((cmd (completing-read "Execute: " (amirreza/list-of-executables)))
           (name (format "External Program: %s" cmd)))
        (start-process-shell-command name name cmd)))

  (global-set-key (kbd "s-d") 'amirreza/run-command)
#+end_src
* Dotfiles
