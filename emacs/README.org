...
* Modules
#+begin_src emacs-lisp
  (defmacro enabled? (mod &rest body)
    (let ((varname (intern (format "%s-enabled?" mod))))
      `(when (boundp ',varname)
         ,@body)))
...
#+begin_src emacs-lisp
 ...
 ;; (use-package doom-modeline :straight t :config (setq doom-modeline-height 35) (doom-modeline-mode 1))
...
#+begin_src emacs-lisp
  ...
  (enabled? swiper (use-package swiper :straight t :bind ("C-s" . swiper)))
...
#+BEGIN_SRC emacs-lisp
  (use-package magit
    ...
    :init
    (enabled? evil (evil-global-set-key 'normal (kbd "SPC v g") 'magit-status))
    :bind
    (("C-x g" . 'magit-status)
