...
#+end_src
** Map some basic keys
Since we are using Evil mode we need to bind some basic functionalities of emacs to more vimish like keys.
#+begin_src emacs-lisp
  (evil-define-key 'normal global-map (kbd "SPC f f") 'find-file)
  (evil-define-key 'normal global-map (kbd "SPC SPC") 'find-file)
#+end_src
** Buffers
#+begin_src emacs-lisp
