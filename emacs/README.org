...
- [[#integrated-tools][Integrated Tools]]
  ...
  - [[#terminal][Terminal]]
  - [[#file-type-search-todo][File Type Search (TODO)]]
- [[#dotfiles][Dotfiles]]
...
** File Type Search
#+begin_src emacs-lisp
  (defvar amirreza/file-type-loc '(:videos '("~/Videos" "~/Downloads") :musics '("~/Music" "~/Downloads")))

  (defun amirreza/where-to-look-for (entity)
    ""
    (let* ((loc (plist-get amirreza/file-type-loc entity))
           (directory-files))))
#+end_src
* Shell
...
#+begin_src emacs-lisp
  ...
  (use-package peep-dired
    ...
    :bind (:map dired-mode-map
                ("P" . peep-dired)))
  (use-package ranger :after evil :straight t :after dired :init (setq ranger-override-dired t))
#+end_src
* Integrated Tools
** Acme functionality
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
(use-package counsel :bind ("s-d" . counsel-linux-app))
#+end_src
** Terminal
#+begin_src emacs-lisp
  (defvar amirreza/terminal "alacritty")
  (use-package counsel :bind ("s-t" . amirreza/terminal)
    :config
...
#+end_src
** File Type Search (TODO)
#+begin_src emacs-lisp
  (defvar amirreza/file-types '(:music :video :doc))
  ...
  (defun amirreza/search-for ()
    (interactive)
    (let* ((filetype (completing-read "FileType: " amirreza/file-types))
           (paths (plist-get amirreza/file-type-loc filetype))
           (program (plist-get amirreza/file-assoc-program filetype))
           (files (mapcar (lambda (path)
                            (directory-files-recursively path directory-files-no-dot-files-regexp))
                          paths))
           (file (completing-read "Open: " files))
           (process-name (format "%s: %s" filetype file)))
      (message "::::::::::: %s" paths)
      (start-process-shell-command process-name process-name (format "%s %s" filetype file))))
