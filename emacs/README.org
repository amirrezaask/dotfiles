...
darkness. 
=SideNote=: I really think Vi way editing is superior but since I'm from Emacs land, to keep my sanity I'm going to continue
with Emacs way.
#+begin_src emacs-lisp
  (use-package evil 
    :disabled t
  ...
  (use-package general
    :straight t
    :config
    (general-create-definer space-leader-lord :prefix ";" :states 'normal :keymaps 'override)
    (general-create-definer space-leader :prefix ";" :states 'normal)
    (space-leader-lord
...
#+begin_src emacs-lisp
    ...
    (use-package helm-rg :straight t :commands helm-rg)
#+end_src
