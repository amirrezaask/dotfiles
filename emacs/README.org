...
#+begin_src emacs-lisp
  ...
  (use-package esh-mode
    ...
    (defun amirreza/eshell-history-search ()
      (interactive)
      (insert (completing-read "History: " (ring-elements eshell-history-ring)))
      (eshell-send-input))
    ...
    (defun amirreza/eshell-find-file-at-point ()
      ...
      (let ((file (ffap-file-at-point)))
        (if file
            (find-file file)
          (user-error "No file at point"))))
    (defmacro amirreza/eshell-defalias-sys (alias command)
      ""
      `(defalias
         ,alias
         (lambda ()
    ...
    (defalias 'd (lambda () (dired default-directory)))
    (amirreza/eshell-defalias-sys 'ocgp "oc get pods")
...
**** VTerm
#+begin_src emacs-lisp
  (use-package vterm :straight t :bind ("<f10>" . vterm))
#+end_src
** IDE setup
