...
* Video
#+begin_src emacs-lisp
  (defun amirreza/open-video (filename)
    (let ((filename (expand-file-name filename)))
      (start-process (format "*VLC: %s*" filename) "*VLC*" "vlc" filename)))

  (defvar amirreza/video-location "~/Videos")
  (defun amirreza/play-movie ()
    (interactive)
    (amirreza/open-video (completing-read "Video: " (directory-files-recursively amirreza/video-location ".*"))))
#+end_src
* Devops Tools
