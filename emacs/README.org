...
*** Doom modeline
#+begin_src emacs-lisp
  (use-package doom-modeline :disabled t :straight t :init (setq doom-modeline-height 30) :config (doom-modeline-mode 1))
#+end_src
** Font
   #+BEGIN_SRC elisp
...
** Projectile
   #+BEGIN_SRC elisp
     ...
     (use-package projectile
       :straight t
       :commands (projectile-find-file projectile-project-root amirreza/find/body amirreza/grep)
       :init
       (with-eval-after-load 'evil
       ...
       :bind
       (:map global-map
             ("C-c f f" . amirreza/find-file)
             ("C-c f p" . amirreza/find-project)
             ("C-c f s" . amirreza/grep))
       :config
       (defun amirreza/grep ()
         (interactive)
         (cond
