...
- [[#ice--integrated-computing-environment-][ICE ( Integrated Computing Environment )]]
  ...
  - [[#file-search][File search]]
    - [[#fzf][Fzf]]
    - [[#find-sub-directory][Find sub directory]]
    - [[#recents][Recents]]
...
#+begin_src emacs-lisp
  (use-package which-key
    ...
    (which-key-add-key-based-replacements "C-c f" "File functions")
...
*** Find sub directory
#+begin_src emacs-lisp
  (defun amirreza/--find-sub-directories-recur ()
        (interactive)
        (let* ((sub-dirs (directory-files-recursively default-directory ".*" t nil nil))
               (sub-dirs-cleaned (seq-filter (lambda (dir-name)
                             (if (string-match-p "\\.git" dir-name) nil t)) sub-dirs)))
          (completing-read "Find Sub directory: " sub-dirs-cleaned)))

  (defun amirreza/find-sub-directories-recur ()
    (interactive)
    (find-file (amirreza/--find-sub-directories-recur)))

(global-set-key (kbd "C-c f d") 'amirreza/find-sub-directories-recur)
#+end_src
*** Recents
#+begin_src emacs-lisp
  (use-package recentf
  ...
  (use-package esh-mode
   ...
   (defun amirreza/eshell-recent-dir-search ()
    ...
    (defun amirreza/eshell-find-sub-dirs-recur ()
      (interactive)
        (insert (amirreza/--find-sub-directories-recur))
        (eshell-send-input))
