...
local function scandir(path, hidden)
	...
	local files_out = {}
	for i, file in pairs(_files) do
		file[1] = string.format("%d", file[1])
			.. string.rep(" ", #string.format("%d", biggest_size) - #string.format("%d", file[1]))
...
function M.open_side_file_browser(path)
	...
	if
		M.SIDE_FILE_BUF == nil
		or M.SIDE_FILE_WIN == nil
		or (not vim.api.nvim_buf_is_loaded(M.SIDE_FILE_BUF) or not vim.api.nvim_win_is_valid(M.SIDE_FILE_WIN))
	then
		vim.cmd([[ set nospr ]])
		vim.cmd([[ vnew ]])
