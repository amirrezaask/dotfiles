...
if [[ "$OSTYPE" == "darwin"* ]]; then
    if test -d "$HOME/Library/Application Support/Cursor"; then
        ...
        ln -s "$(pwd)/vscode-keybindings.json" "$HOME/Library/Application Support/Cursor/User/keybindings.json"
        ln -s "$(pwd)/vscode-settings.json" "$HOME/Library/Application Support/Cursor/User/settings.json"
    fi
